vueReview = new Vue({
    ...
    methods: {
        load: function () {
            ...
            ]).then(() => vueRecordTypes.load())
        },
    }
});

vueRecordTypes = new Vue({
    el: "#createType",
    data: {
        json: "",
        types: [],
...
vueDataManagement = new Vue({
    ...
    methods: {
        exportJSON() {
            ...
            data = {};
            promises = [];
            promises.push(DB.getTypes().then(types => data.types = types));
            promises.push(DB.getRecords({}).then(records => data.records = records));
            promises.push(DB.getTags().then(tags => data.tags = tags));
            Promise.all(promises).then(() => this.json = JSON.stringify(data))
                .then(() =>
