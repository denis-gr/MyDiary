...
argparser.add_argument("--start_url", default="")
argparser.add_argument("--is_not_minify", action="store_true")
...
for path in pathlib.Path(TEMPLATE_DIR).glob("**/*.*"):
    ...
    if ".min" in path.suffixes:
        ...
        min_fun = css_html_js_minify.html_minify
    ...
    if (args.is_not_minify):
        result_fun = lambda x: fun(x)
    else:
        result_fun = lambda x: min_fun(fun(x))

    if is_text:
        with new_path.open("w") as output:
            output.write(result_fun(render_to_string(temp_path, context=CONTEXT)))
    else:
        with new_path.open("wb") as output, path.open("rb") as input:
            output.write(result_fun(input.read()))
